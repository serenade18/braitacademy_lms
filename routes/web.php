...
Route::get('/check-cache-path', function () {
    $cachePath = storage_path('framework/cache/data');
    echo 'Cache path: ' . realpath($cachePath) . '<br>';
    echo 'Permissions: ' . substr(sprintf('%o', fileperms($cachePath)), -4) . '<br>';
    echo 'Is writable: ' . (is_writable($cachePath) ? 'Yes' : 'No') . '<br>';
...
Route::group(['namespace' => 'Web', 'middleware' => ['check_mobile_app', 'impersonate', 'share', 'check_maintenance']], function () {
    ...
    Route::group(['prefix' => 'certificate'], function () {
        ...
        Route::group(['middleware' => 'web.auth'], function () {
            // purchase certifcate route
            Route::post('/direct-payment', 'WebinarController@purchaseCertificate');
        });
    });
